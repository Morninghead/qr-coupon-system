...
const drawElements = async (page, layoutConfig, absX, absY, scaleX, scaleY, assets, employee) => {
    ...
    for (const id in layoutConfig) {
        ...
        const x = pt(absX + (config.x * scaleX)), y = page.getHeight() - pt(absY + (config.y * scaleY));
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const fontBytes = await fs.readFile(fontPath); const thaiFont = await pdfDoc.embedFont(fontBytes);
        ...
        for (const { url, bytes } of fetchedImages) {
            if (bytes) { try { const image = await pdfDoc.embedPng(bytes).catch(() => pdfDoc.embedJpg(bytes)); imageAssetMap.set(url, image); } catch {} }
        }
        ...
        while (i < employees.length) {
            ...
            for (let row = 0; row < maxRow; row++) {
                for (let col = 0; col < maxCol; col++) {
                    ...
                    // "หลังบัตร" (ขวา cell)
                    await drawElements(page, template.layout_config_back, offsetX + pt(card_w), offsetY, scaleX, scaleY, assets, employees[i]);
                    i++;
                }
            }
        }
        const pdfBytes = await pdfDoc.save();
        return { statusCode: 200, headers: { 'Content-Type': 'application/pdf' }, body: Buffer.from(pdfBytes).toString('base64'), isBase64Encoded: true };
    } catch (error) {
